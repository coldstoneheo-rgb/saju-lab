...
유형 귀속 출처: 『삼명통회(三命通會)』 논형(論刑) — 寅巳申 = 무은지형(無恩之刑), 丑戌未 = 지세지형(持勢之刑), 子卯 = 무례지형(無禮之刑), 辰午酉亥 = 자형(自刑). 이설: 한국 교재 일부는 寅巳申·丑戌未의 이름을 서로 바꿔 적는다 — 코어는 고전 배정을 따르고, 유형 코드(`mueun`/`jise`)는 데이터일 뿐 산출(글자·기둥)은 이름과 무관하다.
...
- 같은 글자가 같은 관계 둘 이상에 걸리면(쟁합·투합, 예 甲 둘에 己 하나) 조합 전부 나열 + `shared: true`. `shared`는 **같은 `id`(조·국·종)** 안에서 기둥을 공유할 때만 true — 다른 id는 같은 kind라도 shared가 아니고, 충과 형이 같은 쌍에 있는 것은 중복 나열이지 shared가 아니다. (v1 표에서는 지지 하나가 kind마다 규칙 하나에만 속하므로 kind 기준과 id 기준이 같은 값을 낸다 — 테스트가 이 불변을 고정.)
