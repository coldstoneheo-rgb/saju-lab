...
## L2 — 지장간·십신 (2026-09-22, 5단계)

- `packages/saju-core/src/l2/hidden-stems.data.ts`(정본 `docs/rules/HIDDEN-STEMS.md`, 테스트가 md↔ts 동일성 강제) — 학파 `yeonhae`(기본, 연해자평 월률분야 일수) / `japyeong`(자평진전 인원용사, 일수 없음). 갈림 5칸의 대안 값은 md에 기록만.
- `l2/ten-gods.ts` — `tenGodOf(일간, 천간)` 10코드(오행 5관계 × 음양 동이), 100조합 표 테스트. `tenGodsOfChart(pillars, school)`는 연·월·시간 천간 + 네 지지의 지장간 전부에 십신을 붙인다. 일간 자신은 제외.
- 가중·강약·합충·대운은 이 단계에 없다. 응답은 `options.include`로 요청한 블록만.
