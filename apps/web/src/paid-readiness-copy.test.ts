...
describe("paid readiness copy", () => {
  it("keeps the paid panel framed as a readiness preview instead of live checkout", () => {
    ...
    expect(copy).toContain("개인정보, 환불/문의, 사용 고지 초안을 기준으로 결제 오픈 전 점검을 진행합니다.");
    expect(copy).toContain("결제 실패, 다운로드 실패, 중복 결제 대응 기준을 결제 오픈 전에 정리합니다.");
    ...
    expect(copy).toContain("최종 결제 제공자, 실제 지원 연락처, 사용자 안내 정책 페이지가 확정된 뒤 결제를 엽니다.");
